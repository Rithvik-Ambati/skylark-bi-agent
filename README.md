...
**Hosted app: https://skylark-bi-agent-m8aa.onrender.com/
Decision Log:** [`DECISION\_LOG.md`](./DECISION_LOG.md)
...
Browser (single-page chat UI)
        │  POST /api/chat  { message, session\_id }
        ▼
...
Agent loop (agent.py) ── Claude (Anthropic API), tool-calling
        │
        ├─ list\_deals / aggregate\_deals ─────┐
        ├─ list\_work\_orders / aggregate\_wo ──┤
        ├─ get\_data\_quality\_report ──────────┼──► tools.py
        └─ generate\_leadership\_brief ────────┘        │
                                                        ▼
                                          data\_service.py (schema cache
                                          + orchestration)
                                                        │
                                    ┌───────────────────┴───────────────────┐
                                    ▼                                       ▼
                          monday\_client.py                          normalize.py
...
the original CSVs is hardcoded anywhere in the app. See `monday\_client.py`.
...
2. **Create two boards**, one per dataset:
   ...
   * Import `Deal\_funnel\_Data.xlsx` as a board named `Deals`.
   * Import `Work\_Order\_Tracker\_Data.xlsx` as a board named `Work Orders`.
...
`https://<your-account>.monday.com/boards/<BOARD\_ID>`
...
cd backend
cp .env.example .env
# edit .env: paste MONDAY\_API\_TOKEN, MONDAY\_DEALS\_BOARD\_ID,

\# MONDAY\_WORK\_ORDERS\_BOARD\_ID, ANTHROPIC\_API\_KEY, and
...
backend/
  app/
    ...
    data\_service.py     # Orchestrates monday\_client + normalize, schema caching
    monday\_client.py     # Raw GraphQL client for monday.com API v2
...
of the actual `Deal\_funnel\_Data.xlsx` and `Work\_Order\_Tracker\_Data.xlsx`
files (to ground the cleaning logic in real messiness rather than
assumptions). The app was tested end-to-end against the live monday.com boards after deployment, including verifying correct aggregation, data-quality flagging, and cross-board queries.



# 9\. Challenges faced

\- \*\*Loading 500+ real rows into monday.com via API\*\*: batching GraphQL

&#x20; mutations, handling column-type mismatches, and one gap in the load

&#x20; sequence that required a follow-up pass to catch missed rows.

\- \*\*Anthropic identity-linked API key\*\*: the API key type issued by this

&#x20; organization required an explicit `anthropic-workspace-id` header on

&#x20; every request (not covered in the basic quickstart docs) — resolved

&#x20; by fetching the workspace ID from a response header and wiring it

&#x20; into the client's default headers.

\- \*\*Python 3.14 / dependency compatibility\*\*: pinned package versions

&#x20; in `requirements.txt` didn't have prebuilt wheels for a very new

&#x20; Python version, requiring loosened version constraints to avoid a

&#x20; native compile step that needed a C++ toolchain not present on the

&#x20; dev machine.



## 10\. Known limitations / what's next

See `DECISION\_LOG.md`, section "What I'd do with more time."
